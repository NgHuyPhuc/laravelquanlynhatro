...
if (!function_exists('MenuNguoiThue')) {
    function generateMenuNguoiThue($nhatro, $phongtro, $check)
    {
        $html = '<div class="col-12 grid-margin stretch-card">';
        $html .= '<div class="card">';
        $html .= '<div class="row">';
        ...
        $html .= '<br/><hr/>';
        $html .= '<a href="' . route('phongtro.nguoithue.themnguoi', ['id' => $nhatro->id, 'id_phong' => $phongtro->id]) . '" class="btn btn-info">Thêm mới người thuê phòng</a>';
        $html .= '<a href="' . route('phong.nguoithue.danhsach.all', ['id' => $nhatro->id, 'id_phong' => $phongtro->id]) . '" class="btn btn-info">Danh sách tất cả người từng thuê</a>';
        // Nút ẩn (Xóa phòng)
        $html .= '<button hidden type="button" class="btn btn-danger float-right" data-toggle="modal" data-target="#exampleModal">Xóa phòng</button>';
