                            ...
                            <div class="card-body">
                                ...
                                <div class="template-demo">
                                  <a href="{{route('phongtro.nguoithue.themnguoi',['id' => $nhatro->id, 'id_phong' => $phong->id])}}" class="btn btn-info">Thêm mới người
                                      thuê phòng</a>
                                  <a href="{{route('nhatro.phong.nguoithue.show.all.info',['id' => $id, 'id_phong' => $phong->id])}}" class="btn btn-info">Xem thông tin người thuê phòng</a>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card-body">
                                ...
                                <form method="POST" action="{{route('phongtro.suaphong.post',['id' => $id , 'id_phong' => $phong->id])}}" class="forms-sample" enctype="multipart/form-data">
                                  @csrf
                                    <div class="form-group">
                                    <label for="exampleInputName1">Tên phòng</label>
                                    <input name="ten_phong" type="text" class="form-control" id="exampleInputName1" placeholder="Giá phòng" value="{{$phong->ten_phong}}">
                                  </div>
                                  <div class="form-group">
                                    ...
                                    <select style="color: black" name="id_tang" class="form-control form-control-sm"
                                        ...
                                        @foreach ($tang as $item)
                                            <option value="{{$item->id}}" {{$item->id == $phong->id_tang ? 'selected' : ''}}>{{$item->ten_tang}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                  <div class="form-group">
                                    <label for="exampleInputName1">Giá phòng</label>
                                    <input name="gia_phong" type="text" class="form-control" id="exampleInputName1" placeholder="Giá phòng" value="{{$phong->gia_phong}}">
                                  </div>
                                  <div class="form-group">
                                    <label for="exampleInputEmail3">Dùng mạng:</label>
                                    <label class="toggle-switch toggle-switch-success">
                                        <input name="dung_mang" type="checkbox" value="{{$phong->dung_mang}}" {{$phong->dung_mang == 1 ? 'checked' : ''}} >
                                  ...
                                  <div class="form-group">
                                    ...
                                    <label class="toggle-switch toggle-switch-success">
                                        <input name="trang_thai" type="checkbox" value="{{$phong->trang_thai}}" {{$phong->trang_thai == 1 ? 'checked' : ''}}>
                                        <span class="toggle-slider round"></span>
                                      </label> 
                                  </div>
                                  <div class="form-group">
